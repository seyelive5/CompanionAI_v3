...
namespace CompanionAI_v3.Analysis
{
    ...
    /// BattlefieldInfluenceMap과 동일한 그리드 시스템 사용 (CELL_SIZE = 2.0f).
    ...
    public class PredictiveThreatMap
    {
        ...
        /// <summary>그리드 셀 크기 (BattlefieldInfluenceMap과 동일)</summary>
        ...
        private float CalculatePredictedThreatAt(Vector3 position)
        {
            ...
            foreach (var mobility in _enemyMobilities)
            {
                ...
                // 1. 현재 위치에서의 거리
                float distFromEnemy = Vector3.Distance(position, mobility.Enemy.Position);
                ...
                if (mobility.HasGapCloser && distFromEnemy <= mobility.GapCloserRange + mobility.MovementPoints)
                {
                    // GapCloser 범위 내면 추가 위협
                    gapCloserThreat = 0.5f * (1f - distFromEnemy / (mobility.GapCloserRange + mobility.MovementPoints));
