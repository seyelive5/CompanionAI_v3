...
namespace CompanionAI_v3.Analysis
{
    ...
    public static class EnemyMobilityAnalyzer
    {
        ...
        /// <summary>
        /// 휴리스틱으로 GapCloser 능력 감지
        /// (Blueprint에 Charge, Teleport, Jump 등 포함)
        /// </summary>
        private static bool IsGapCloserHeuristic(AbilityData ability)
        {
            try
            {
                ...
                if (name.Contains("charge") || name.Contains("돌격") ||
                    name.Contains("leap") || name.Contains("도약") ||
                    name.Contains("teleport") || name.Contains("순간이동") ||
                    name.Contains("rush") || name.Contains("돌진") ||
                    name.Contains("dash") || name.Contains("질주"))
                {
                    return true;
                }

                // Component 기반 휴리스틱 (이동 강제 능력)
