...
namespace CompanionAI_v3.Analysis
{
    ...
    public class SequenceOptimizer
    {
        ...
        private ActionSequence CreateRetreatThenAttackSequence(AbilityData attack, BaseUnitEntity target, float remainingMP)
        {
            ...
            // ★ v3.0.58: 실제 능력 사거리 사용 (30m 하드코딩 제거)
            float abilityRange = CombatAPI.GetAbilityRange(attack);
            if (abilityRange <= 0) abilityRange = 15f;  // 폴백: 기본 15m

            if (!attack.IsMelee && distAfterRetreat > abilityRange)
            {
                Main.LogDebug($"[{_logPrefix}] Retreat rejected: {attack.Name} range={abilityRange:F0}m, distance after retreat={distAfterRetreat:F1}m");
                return null;
            }
