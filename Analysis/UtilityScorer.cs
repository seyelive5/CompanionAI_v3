            ...
            if (AbilityDatabase.IsAoE(attack))
            {
                ...
                if (useAoEOptimization && situation.Enemies != null && situation.Enemies.Count >= 2)
                {
                    ...
                    if (clusters.Count > 0)
                    {
                        ...
                        score += bestCluster.Count * 15f;       // 적당 +15
                        score += bestCluster.Density * 20f;     // 밀도 보너스
                    }
                    else
                    {
                        int nearbyEnemies = CountEnemiesNear(target, situation, aoERadius);
