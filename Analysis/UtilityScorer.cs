...
namespace CompanionAI_v3.Analysis
{
    ...
    public static class UtilityScorer
    {
        ...
        public static CombatPhase DetectPhase(Situation situation)
        {
            ...
            if (allyAvgHP < 35f || situation.HPPercent < 25f)
                return CombatPhase.Desperate;

            // 정리 단계: 적이 2명 이하
            if ((situation.Enemies?.Count ?? 0) <= 2)
                return CombatPhase.Cleanup;

            // 초반: 첫 턴 또는 버프 전
            if (!situation.HasBuffedThisTurn && !situation.HasAttackedThisTurn && situation.CurrentAP >= 3f)
        ...
        public static float ScoreBuff(AbilityData buff, Situation situation)
        {
            ...
            // ★ AP 효율: 비용 대비 효과 (낮은 비용 = 좋음, 하지만 절대적이지 않음)
            float cost = CombatAPI.GetAbilityAPCost(buff);
            if (cost <= 0f) score += 15f;       // 0AP = 보너스
            else if (cost <= 1f) score += 10f;  // 1AP = 좋음
            else if (cost >= 3f) score -= 10f;  // 고비용 = 감점
            ...
            switch (timing)
            {
                ...
                case AbilityTiming.SelfDamage:
                    // HP가 충분할 때만
                    if (situation.HPPercent >= 70f) score += 10f;
                    else if (situation.HPPercent < 50f) score -= 30f;
                    break;
                ...
                case AbilityTiming.Taunt:
                    ...
                    int nearbyEnemies = situation.Enemies?.Count(e =>
                        e != null && CombatAPI.GetDistance(situation.Unit, e) <= 5f) ?? 0;
        ...
        public static float ScoreAttack(AbilityData attack, BaseUnitEntity target, Situation situation)
        {
            ...
            if (estimatedDamage >= targetHP)
                score += 80f;
            // 2타 킬 가능 = 높은 점수
            else if (estimatedDamage >= targetHP * 0.5f)
                score += 40f;
            // 데미지 비율 기반
            else
                score += (estimatedDamage / Math.Max(targetHP, 1f)) * 50f;

            // ★ AP 효율
            float cost = CombatAPI.GetAbilityAPCost(attack);
            float damagePerAP = estimatedDamage / Math.Max(cost, 0.5f);
            score += Math.Min(damagePerAP * 2f, 30f);  // 최대 30점 보너스

            // ★ 거리 적합성
            float distance = CombatAPI.GetDistance(situation.Unit, target);
            if (attack.IsMelee)
            {
                if (distance <= 2f) score += 15f;   // 근접 범위 = 좋음
                else score -= distance * 3f;        // 멀면 감점
            }
            else
            {
                // 원거리: 적정 거리에서 보너스
                if (distance >= 5f && distance <= 15f) score += 10f;
                else if (distance < 3f) score -= 10f;  // 너무 가까우면 감점
            }
            ...
            {
                if (targetHPPercent <= 30f) score += 40f;  // 마무리 대상
                else score -= 30f;  // 마무리 아니면 비효율
            }

            // ★ AoE 보너스: 범위 공격이면 근처 적 수에 따라 보너스
            if (AbilityDatabase.IsAoE(attack))
            {
                int nearbyEnemies = CountEnemiesNear(target, situation, 5f);
                score += nearbyEnemies * 15f;  // 적 1명당 15점
            }

            // ★ 위험한 AoE 감점
            if (AbilityDatabase.IsDangerousAoE(attack))
            {
                int nearbyAllies = CountAlliesNear(target, situation, 5f);
                score -= nearbyAllies * 30f;  // 아군 피해 위험
            }
        ...
        public static float ScoreTarget(BaseUnitEntity target, Situation situation)
        {
            ...
            score += (100f - hpPercent) * 0.5f;  // 최대 50점

            // ★ 1타 킬 가능성 (최우선)
            if (situation.PrimaryAttack != null && CombatAPI.CanKillInOneHit(situation.PrimaryAttack, target))
                score += 60f;

            // ★ 위협도 평가
            float threat = EvaluateThreat(target, situation);
            score += threat * 30f;  // 최대 30점

            // ★ 거리 (가까울수록 좋음 - 이동 비용 절약)
            float distance = CombatAPI.GetDistance(situation.Unit, target);
            score -= distance * 2f;
        ...
        public static float ScoreHeal(AbilityData heal, BaseUnitEntity target, Situation situation)
        {
            ...
            // HP 낮을수록 힐 가치 높음
            if (targetHP < 25f) score += 80f;       // 위급
            else if (targetHP < 50f) score += 50f;  // 낮음
            else if (targetHP < 75f) score += 20f;  // 보통
            else score -= 30f;                       // 힐 불필요

            // ★ 자기 힐 vs 아군 힐
            bool isSelfHeal = target == situation.Unit;
            if (isSelfHeal && situation.HPPercent < 30f)
                score += 40f;  // 자기가 위급하면 자기 힐 우선
