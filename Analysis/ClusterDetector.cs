...
namespace CompanionAI_v3.Analysis
{
    ...
    public class EnemyCluster
    {
        ...
        /// <summary>유효한 클러스터인가? (2명 이상)</summary>
    ...
    public static class ClusterDetector
    {
        ...
        /// <summary>기본 클러스터 탐색 반경</summary>
        public const float DEFAULT_CLUSTER_RADIUS = 6f;
        ...
        /// <param name="minClusterSize">클러스터 최소 크기</param>
        ...
        {
            if (enemies == null || enemies.Count < minClusterSize)
            ...
            try
            {
                ...
                if (validEnemies.Count < minClusterSize)
                    return new List<EnemyCluster>();

                // Greedy 클러스터링: 가장 밀집된 지점 찾기 → 확장 → 반복
                var clusters = GreedyCluster(validEnemies, maxClusterRadius, minClusterSize);
            ...
            List<BaseUnitEntity> allies)
        {
            ...
            try
            {
                ...
                // 시전자 사거리 내 클러스터만 필터링
                clusters = clusters
                    .Where(c => Vector3.Distance(caster.Position, c.Center) <= abilityRange)
                    .ToList();
                ...
                foreach (var cluster in clusters)
                {
                    ApplyAllyPenalty(cluster, allies, caster, aoERadius);
                }
        ...
        public static Vector3? FindOptimalAoEPosition(
            ...
            float aoERadius)
        {
            if (cluster == null || cluster.Count < 2 || ability == null || caster == null)
                return null;
            ...
            try
            {
                ...
                for (float dx = -searchRadius; dx <= searchRadius && positionsChecked < MAX_POSITIONS; dx += searchStep)
                {
                    for (float dz = -searchRadius; dz <= searchRadius && positionsChecked < MAX_POSITIONS; dz += searchStep)
                    {
                        ...
                        // 능력 사거리 체크
                        if (Vector3.Distance(caster.Position, testPos) > abilityRange)
                            continue;

                        // 적중 수 계산
                        int hits = CountEnemiesInRadius(testPos, cluster.Enemies, aoERadius);

                        // 아군 안전 체크
                        int allyHits = CountAlliesInRadius(testPos, allies, caster, aoERadius);
                        if (allyHits >= 2)
                            continue;  // 2명 이상 아군 피해 시 제외
            ...
            float radius)
        {
            return enemies.Count(e =>
                e != seed &&
                !assigned.Contains(e) &&
                Vector3.Distance(seed.Position, e.Position) <= radius);
        }

        /// <summary>
        /// 시드에서 클러스터 확장 (도달 가능한 모든 적 포함)
        /// </summary>
            ...
            float maxRadius)
        {
            ...
            while (toProcess.Count > 0)
            {
                ...
                foreach (var enemy in enemies)
                {
                    ...
                    // 클러스터 중심에서 거리 체크
                    if (Vector3.Distance(clusterCenter, enemy.Position) <= maxRadius)
                    {
            ...
            float aoERadius)
        {
            ...
            foreach (var ally in allies)
            {
                ...
                float dist = Vector3.Distance(cluster.Center, ally.Position);
                if (dist <= aoERadius)
                {
                    try
                    {
                        if (!caster.IsPlayerEnemy && ally.IsInPlayerParty)
                        {
                            ...
                            // 2명 이상 플레이어 파티 아군 = 클러스터 무효화
                            if (playerPartyAlliesInRange >= 2)
                            {
                                cluster.QualityScore = -1000f;
                                Main.LogDebug($"[ClusterDetector] Cluster invalidated: 2+ player allies in range");
                                return;
                            }

                            // 1명 = 큰 페널티
                            cluster.QualityScore -= 60f;
                        }
                        else
                        {
                            // NPC 아군 = 작은 페널티
                            cluster.QualityScore -= 20f;
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        /// 반경 내 적 수 계산
        /// </summary>
        private static int CountEnemiesInRadius(Vector3 center, List<BaseUnitEntity> enemies, float radius)
        {
            return enemies.Count(e =>
                e != null && e.IsConscious &&
                Vector3.Distance(center, e.Position) <= radius);
        }

        /// <summary>
        /// 반경 내 플레이어 파티 아군 수 계산
        /// </summary>
            ...
            float radius)
        {
            ...
            return allies.Count(a =>
                ...
                Vector3.Distance(center, a.Position) <= radius);
        }
